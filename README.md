...
| APLC/TABLES                            | A table file neede by LisaCalc and LisaGraph that I also grabbed off the LisaCalc install diskette. |
...
## Actual Code Changes
- In APHP/HP.TEXT, find the 3 occurrances of "{t12}" and replace with "{t100}".
- In APCL/CLOCK.TEXT, find "{t13}" and replace with "{t101}".
- In APLL/INITFEX.TEXT, find the two occurrances of "{t5}" and replace with "{t103}".
...
- In APLW/TESTBOX.TEXT, we also need to do stuff to disable the spellchecker. First comment out the USES statement lines containing "$U ApLW/UnitSpell.obj", "$U aplw/sp/spelling.obj", and "$U aplw/sp/verify.obj". Then find and comment out the lines containing "fTstSpell", "fTstHeap", "ord (HMReturn)", and "spstatus". And similar UNITLOTUS, find the line "HMReturn := SpTerminate(cleanUp);" and replace it with "HMReturn := ok;". And then do the same thing that we did for UNITLOTUS where we made a new TYPE heading and declared that TSpReturn type underneath it.
...
- In APLC/LCFILER.TEXT, replace the occurrance of "{t2}" with "{t108}" and the occurrance of "{t3}" with "{t107}".
- In APLC/MM/LEX.TEXT, replace the occurrance of "{t2}" with "{t108}" and the occurrance of "{t3}" with "{t107}".
...
- CDCONFIG.OBJ - Allows you to add and remove configrable driver entries from the SYSTEM.cdd configurable driver directory. Source is in SOURCE.
...
-- One is network, which offers a (very limited) peek into the network-enabled future of LOS if set to TRUE. It just shows a "file cabinet" icon on the desktop that I guess was suppoed to be an inbox/outbox thing, but you can't copy files into this folder. And setting this flag to true also corrupts the filesystem and makes a new copy of the Clock on the desktop each time you reboot for some reason, so I'd advide against using it other than just as a curiosity!
